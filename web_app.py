...
def dashboard():
    ...
    # Popup için tüm sınıfların öğrenci listesi (müfettiş atama)
    popup_ogrenciler = []
...
def tik_at(ogrenci_id):
    ...
    # Yeni bir esligi gecti mi? (3, 6, 9, 12)
    onceki = yeni - 1
    yeni_seviye = None
    ...
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return jsonify({
            ...
            "emoji":       d["emoji"],
            "etiket":      d["etiket"],
            "yeni_seviye": yeni_seviye,   # None veya "Uyari" / "Veli Bilgilendirme" vb.
        })
...
# ══════════════════════════════════════════════════════════════════════════
# Öğrenci Görüntüleme (şifresiz, salt okunur)
# ══════════════════════════════════════════════════════════════════════════

@app.route("/ogrenci")
def ogrenci_gorunum():
    """Öğrenciler için şifresiz, salt-okunur panel."""
    ogrenciler = _ogrencilere_durum_ekle(tum_okul_ogrencileri())
    mufettis_veri = bugun_mufettis()
    # Müfettiş adını sadece sonuç açıklanmışsa göster
...
def api_yayin_ogrenciler():
    """Tum okuldaki ogrencileri tik sayisina gore sirali dondurur."""
...
def lig_mac_oyla(mac_id):
    ...
    # Sadece var incelemesi hakemleri oy verebilir
    ogretmen_adi = session["ogretmen_adi"]
...
def api_ogrenci_kadro(sinif_id):
    """Öğrenci paneli için şifresiz kadro görüntüleme."""
    return jsonify(sinif_kadro_getir(sinif_id))


@app.route("/api/ogrenci/ittifaklar")
def api_ogrenci_ittifaklar():
    """Bugünkü tüm ittifakları şifresiz döner (talep aşamasındakiler dahil)."""
...
def api_sinif_ogrencileri_json(sinif_id):
    """Kart modali icin sinif ogrenci listesi."""
...
def api_lig_maclar_ve_tablo():
    """Yayın ekrani icin bugunku maclar + puan tablosu + kadro listesi."""
    maclar = bugun_maclar()
    tablo  = lig_puan_tablosu()

    # Her macin her sinifi icin kadro getir
    sinif_id_seti = set()
    for m in maclar:
...
def api_ittifak_onayla():
    """Öğretmen, öğrenci ittifak talebini onaylar."""
    ittifak_id = int(request.form["ittifak_id"])
...
def api_ittifak_reddet():
    """Öğretmen, öğrenci ittifak talebini reddeder."""
    ittifak_id = int(request.form["ittifak_id"])
...
def api_ogrenci_ittifak_talep():
    """Öğrenci kaynaklı ittifak talebi — şifresiz."""
    try:
        sinif1_id = int(request.form["sinif1_id"])
        sinif2_id = int(request.form["sinif2_id"])
    ...
    if sinif1_id == sinif2_id:
        return jsonify({"ok": False, "sebep": "Aynı sınıf seçilemez"})
...
def api_mufettis_belirle():
    """Sadece yetkili öğretmenler bugünün müfettişini belirler."""
...
# ══════════════════════════════════════════════════════════════════════════
# Taktik Tahtası (şifresiz)
# ══════════════════════════════════════════════════════════════════════════

# ══════════════════════════════════════════════════════════════════════════
# Quiz — Bilgi Yarışması (şifresiz)
# ══════════════════════════════════════════════════════════════════════════
...
def api_quiz_cevapla():
    """Cevapları sunucuda kontrol et, puanla, lig puanını güncelle."""
    veri = request.json or {}
    sinif_id = veri.get("sinif_id")
    sinif_seviyesi = veri.get("sinif_seviyesi")
    ders = veri.get("ders")
    cevaplar = veri.get("cevaplar", {})  # {soru_id: verilen_cevap}
...
def taktik(sinif_id):
    ...
    # Sınıf adı + öğrenci listesini doğrudan çek (id garantili)
    from database import _conn, _kadro_tablosu_olustur
    con = _conn()
    sinif_row = con.execute("SELECT sinif_adi FROM siniflar WHERE id=?",
                            (sinif_id,)).fetchone()
    sinif_adi = sinif_row["sinif_adi"] if sinif_row else f"Sınıf {sinif_id}"

    # Tüm öğrenciler + kayıtlı mevkileri (kadro_ogrenci tablosu varsa)
    try:
    ...
    # mevki adını ekle — MEVKILER = [(no, emoji, ad), ...]
    mevki_map = {m[0]: m[2] for m in MEVKILER}  # {no: ad}
    for o in kadro:
        o["mevki"] = mevki_map.get(o.get("mevki_no", 1), "Santrafor (Golcü)")

    # MEVKILER → template'e list olarak gönder: [{no, emoji, ad}]
...
def api_taktik_kaydet(sinif_id):
    ...
    if sonuc.get("ok"):
        try:
            ...
            # Rolleri kadro_ogrenci tablosuna da yaz
...
# ══════════════════════════════════════════════════════════════════════════
# Veri Sıfırlama — Admin
# ══════════════════════════════════════════════════════════════════════════

SIFIRLAMA_SIFRESI = "ERENLER2024"   # Yanlışlıkla tıklamaya karşı koruma
...
def api_admin_sifirla():
    ...
    if girilen != SIFIRLAMA_SIFRESI:
        return jsonify({"ok": False, "sebep": "Şifre yanlış!"}), 403
